...
class Comments extends BaseController {
    ...
    function edit(int $picture_index, int $comment_index) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!isset($comment_form['__errors'])) {
                ...
                \Helpers\redirect('/' . $picture_index . \Helpers\get_GET_params(['page', 'filter', 'ref']));
            }
        } else {
            $comments = new \Models\Comment();
    ...
    function delete(int $picture_index, int $comment_index) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $comments = new \Models\Comment();
            $comment = $comments->get_or_404($comment_index, 'comments.id', 'users.name AS user_name, contents, uploaded', ['users']);
            $ctx = ['comment' => $comment,
                'picture' => $picture_index,
