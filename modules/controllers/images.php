...
class Images extends BaseController {
    function list() {
        ...
        $picts->select('pictures.id, title, filename, uploaded, users.name AS user_name, categories.name AS cat_name, categories.slug, (SELECT COUNT(*) FROM comments WHERE comments.picture = pictures.id) AS comment_count', ['users', 'categories'], '', NULL, '', 0, 3);
        $ctx = [
            'cats' => $cats, 
            'picts' => $picts
        ];
    ...
    function item(int $index) {
        ...
        $picts = new \Models\Picture();
        $pict = $picts->get_or_404($index, 'pictures.id', 'pictures.id, title, description, filename, uploaded, users.name AS user_name, categories.name AS cat_name, categories.slug, (SELECT COUNT(*) FROM comments WHERE comments.picture = pictures.id) AS comment_count', ['users', 'categories']);
        $comments = new \Models\Comment();
        $comments->select('comments.id, contents, users.name AS user_name, uploaded, users.id AS user_id', ['users'], 'picture = ?', [$index]);
        $ctx = [
            'pict' => $pict, 
    ...
    function by_cat(string $slug) {
        ...
        $picts = new \Models\Picture();
        $pict_count_rec = $picts->get_record('COUNT(*) AS cnt', NULL, $w, $p);
        $paginator = new \Paginator($pict_count_rec['cnt'], ['filter']);
        $picts->select('pictures.id, title, filename, uploaded, users.name AS user_name, (SELECT COUNT(*) FROM comments WHERE comments.picture = pictures.id) AS comment_count', ['users'], $w, $p, '', $paginator->first_record_num, \Settings\RECORDS_ON_PAGE);
        $ctx = [
            'cat' => $cat, 
            'picts' => $picts,
            'paginator' => $paginator,
    ...
    function by_user(string $username) {
        ...
        $pict_count_rec = $picts->get_record('COUNT(*) AS cnt', NULL, $w, $p);
        $paginator = new \Paginator($pict_count_rec['cnt'], ['filter']);
        $picts->select('pictures.id, title, filename, uploaded, categories.name AS cat_name, categories.slug, (SELECT COUNT(*) FROM comments WHERE comments.picture = pictures.id) AS comment_count, pictures.user', ['categories'], $w, $p, '', $paginator->first_record_num, \Settings\RECORDS_ON_PAGE);
        $ctx = [
            'user' => $user, 
            'picts' => $picts,
            'paginator' => $paginator,
    ...
    private function check_user(int $picture_index) {
        $pictures = new \Models\Picture();
        $picture = $pictures->get_or_404($picture_index, 'id', 'user');
        if (!($this->current_user && ($this->current_user['id'] == $picture['user'] || $this->current_user['admin']))) {
            throw new \Page403Exception();
        }
    }
    ...
    function edit(string $username, int $picture_index) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $picture_form = \Forms\Picture2::get_normalized_data($_POST);
            if (!isset($picture_form['__errors'])) {
                $picture_form = \Forms\Picture2::get_prepared_data($picture_form);
                $pictures = new \Models\Picture();
                $pictures->update($picture_form, $picture_index);
                \Helpers\redirect('/users/' . $username . \Helpers\get_GET_params(['page', 'filter']));
            }
        } else {
            $pictures = new \Models\Picture();
            $picture = $pictures->get_or_404($picture_index);
            $picture_form = \Forms\Picture2::get_initial_data($picture);
        }
        $categories = new \Models\Category();
        $categories->select();
        $ctx = [
    ...
    function delete(string $username, int $picture_index) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $pictures = new \Models\Picture();
            $picture = $pictures->get_or_404($picture_index, 'id', 'title, uploaded');
            $ctx = [
                'picture' => $picture, 
                'username' => $username,
