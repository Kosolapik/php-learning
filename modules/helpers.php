...
namespace Helpers {
    ...
    function show_errors(string $fld_name, array $form_data) {
        if (isset($form_data['__errors'][$fld_name])) {
            echo '<div class="error">' . $form_data['__errors'][$fld_name] . '</div>';
        }
    }
    ...
    function get_thumbnail(string $filename) {
        global $base_path;
        $thumb_file_name = pathinfo($filename, PATHINFO_FILENAME) . '.jpg';
        $thumb_path = $base_path . \Settings\THUMBNAIL_FILE_PATH . $thumb_file_name;
        if (!file_exists($thumb_path)) {
            ...
            $height = imagesy($img);
            imagefilledrectangle($img, 0, $height - 10, 100, $height, $stwhite);
    ...
    function delete_thumbnail(string $filename) {
        global $base_path;
        $thumb_path = $base_path . \Settings\THUMBNAIL_FILE_PATH . pathinfo($filename, PATHINFO_FILENAME) . '.jpg';
        if (file_exists($thumb_path)) {
            unlink($thumb_path);
        }
    }
