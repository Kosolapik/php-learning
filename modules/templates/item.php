...
<H4>
    Категория: <a href="/cats/<?php echo $pict['slug'] ?>/"><?php echo $pict['cat_name'] ?></a>
</h4>
...
<?php foreach ($comments as $comment) { ?>
    ...
    <p>Опубликован: <?php echo \Helpers\get_formatted_timestamp($comment['uploaded']) ?></p>
    <?php if ($__current_user && ($__current_user['id'] == $comment['user_id'] || $__current_user['admin'])) { ?>
    <?php $u1 = '/' . $pict['id'] . '/comments/' . $comment['id'] ?>
    <p>
        <a href="<?php echo $u1 . '/edit' . $u2 ?>">Исправить</a>
        <a href="<?php echo $u1 . '/delete' . $u2 ?>">Удалить</a>
    </p>
    <?php } ?>
    <p>&nbsp;</p>
<?php } ?>
